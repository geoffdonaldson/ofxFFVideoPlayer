...
bool ofxFFMPEGVideoPlayer::loadMovie(string name)
{
    ...
    rgbImage.initWithTexture(getWidth(), getHeight(), GL_RGBA);
//    openCL.loadKernel("convert_b_yuv420p_i_rgb");
//    openCLkernel = openCL.kernel("convert_b_yuv420p_i_rgb");
    
    openCL.loadKernel("convert_bbb_yuv420p_i_rgb");
    openCLkernel = openCL.kernel("convert_bbb_yuv420p_i_rgb");
    
    yuvImage.resize(3);
    yuvImageLineSize.resize(3);
    
    yuvImageLineSize[0] = 0;
    yuvImageLineSize[1] = 0;
    yuvImageLineSize[2] = 0;
            ...
            if(isFrameDecoded)
            {
                switch (pix_conv_type) {
                    ...
                    case OPENCL:
                        ...
                        m_VideoTex = rgbImage.getTexture();
                        break;
                }
            }
        }
...
ofTexture * ofxFFMPEGVideoPlayer::getTexture()
{
    ...
    return &m_VideoTex; //&rgbImage.getTexture();
}
...
bool ofxFFMPEGVideoPlayer::openVideoStream()
{
	...
	//Initialize Context
	m_pSwScalingContext = sws_getContext(getWidth(), getHeight(), m_pVideoCodecContext->pix_fmt, getWidth(), getHeight(), PIX_FMT_RGB24, SWS_BILINEAR /*| SWS_ACCURATE_RND*/, NULL, NULL, NULL);
...
bool ofxFFMPEGVideoPlayer::decodeVideoFrame(AVPacket* pAVPacket)
{
    ...
    switch (pix_conv_type) {
        ...
        case OPENCL:
            ...
            if (yuvImageLineSize[0] != m_pVideoFrame->linesize[0]){
                yuvImage[0].initBuffer((m_pVideoFrame->linesize[0]*getHeight()*sizeof(unsigned char)));
                yuvImageLineSize[0] = m_pVideoFrame->linesize[0];
            }
            if (yuvImageLineSize[1] != m_pVideoFrame->linesize[1]){
                yuvImage[1].initBuffer((m_pVideoFrame->linesize[1]*(getHeight()/2)*sizeof(unsigned char)));
                yuvImageLineSize[1] = m_pVideoFrame->linesize[1];
            }
            if (yuvImageLineSize[2] != m_pVideoFrame->linesize[2]){
                yuvImage[2].initBuffer((m_pVideoFrame->linesize[2]*(getHeight()/2)*sizeof(unsigned char)));
                yuvImageLineSize[2] = m_pVideoFrame->linesize[2];
            }
            
            //    cout << "1 Size is '" << (long long)(m_pVideoFrame->linesize[0]*getHeight()*sizeof(unsigned char)) << "' .." << endl;
            //    cout << "2 Size is '" << (long long)(m_pVideoFrame->linesize[1]*(getHeight()/2)*sizeof(unsigned char)) << "' .." << endl;
            //    cout << "3 Size is '" << (long long)(m_pVideoFrame->linesize[2]*(getHeight()/2)*sizeof(unsigned char)) << "' .." << endl;
            
            yuvImage[0].write(m_pVideoFrame->data[0], 0, m_pVideoFrame->linesize[0]*getHeight()*sizeof(unsigned char));
            yuvImage[1].write(m_pVideoFrame->data[1], 0, m_pVideoFrame->linesize[1]*(getHeight()/2)*sizeof(unsigned char));
            yuvImage[2].write(m_pVideoFrame->data[2], 0, m_pVideoFrame->linesize[2]*(getHeight()/2)*sizeof(unsigned char));
            
            uint w = getWidth();
            uint h = getHeight();
            openCLkernel->setArg(0, yuvImage[0].getCLMem());
            openCLkernel->setArg(1, yuvImageLineSize[0]);
            openCLkernel->setArg(2, yuvImage[1].getCLMem());
            openCLkernel->setArg(3, yuvImageLineSize[1]);
            openCLkernel->setArg(4, yuvImage[2].getCLMem());
            openCLkernel->setArg(5, yuvImageLineSize[2]);
            openCLkernel->setArg(6, w);
            openCLkernel->setArg(7, h);
            openCLkernel->setArg(8, rgbImage.getCLMem());
            openCLkernel->run2D(getWidth(), getHeight());
